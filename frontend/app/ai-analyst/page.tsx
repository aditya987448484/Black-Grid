...
import type { AnalystReport } from "@/components/ai-analyst/AnalystWorkspace";
...
function renderMarkdown(text: string): React.ReactNode {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("### ")) {
      els.push(<h4 key={key++} style={{ fontSize: 13, fontWeight: 600, color: "#cbd5e1", margin: "10px 0 4px" }}>{renderInline(line.slice(4), `h4${key}`)}</h4>);
      i++; continue;
    }
    if (line.startsWith("## ")) {
      els.push(<h3 key={key++} style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0", margin: "12px 0 4px" }}>{renderInline(line.slice(3), `h3${key}`)}</h3>);
      i++; continue;
    }
    if (line.startsWith("# ")) {
      els.push(<h2 key={key++} style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", margin: "14px 0 6px" }}>{renderInline(line.slice(2), `h2${key}`)}</h2>);
      i++; continue;
    }
...
function EmptyState({ onChipClick }: { onChipClick: (t: string) => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16, padding: 24 }}>
      <div style={{ width: 52, height: 52, borderRadius: 12, background: "rgba(255,255,255,0.06)", border: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>
        📈
      </div>
      <div style={{ textAlign: "center" }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "#f1f5f9", margin: 0 }}>AI Analyst</h2>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", marginTop: 4, marginBottom: 0 }}>Powered by DeepSeek</p>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginTop: 4 }}>
        {SUGGESTIONS.map((s) => (
          <button
            ...
            onMouseEnter={(e) => { const b = e.currentTarget; b.style.background = "rgba(255,255,255,0.08)"; b.style.color = "#e2e8f0"; }}
            onMouseLeave={(e) => { const b = e.currentTarget; b.style.background = CARD; b.style.color = "#94a3b8"; }}
          >
            {s}
          </button>
        ))}
...
function MessageBubble({ msg, onViewReport }: { msg: Message; onViewReport: (r: AnalystReport) => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: isUser ? "row-reverse" : "row", alignItems: "flex-start", gap: 10, marginBottom: 16 }}>
      {!isUser && (
        <div style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(255,255,255,0.06)", border: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0, marginTop: 2 }}>
          📈
        </div>
      )}
      <div style={{ maxWidth: "72%", minWidth: 40 }}>
        <div style={{ background: isUser ? "#1e3a5f" : "rgba(255,255,255,0.04)", border: `1px solid ${isUser ? "rgba(30,90,160,0.45)" : BORDER_SUB}`, borderRadius: isUser ? "12px 4px 12px 12px" : "4px 12px 12px 12px", padding: "10px 14px" }}>
          {msg.isTyping ? <TypingIndicator /> : renderMarkdown(msg.content)}
        </div>

        {isUser && msg.attachments && msg.attachments.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6, justifyContent: "flex-end" }}>
            {msg.attachments.map((a) => (
              <span key={a.id} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 4, padding: "2px 8px", fontSize: 11, color: "#94a3b8" }}>
                📎 {a.filename}
              </span>
            ))}
          </div>
        ...
        {!isUser && msg.reportData && !msg.reportLoading && (
          <button
            ...
            style={{ marginTop: 8, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.28)", borderRadius: 6, padding: "5px 12px", color: "#4ade80", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}
...
function RightPanel({ report, onClose }: { report: AnalystReport; onClose: () => void }) {
  ...
  const scrollCSS: CSS = { flex: 1, overflowY: "auto", padding: 16, scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.07) transparent" };

  return (
    <div className="ai-panel-slide" style={{ width: 400, flexShrink: 0, background: CARD, borderLeft: `1px solid ${BORDER}`, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {/* Header */}
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button
            ...
            style={{ background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.28)", borderRadius: 6, padding: "5px 12px", color: "#4ade80", fontSize: 12, cursor: exporting ? "not-allowed" : "pointer", opacity: exporting ? 0.6 : 1 }}
          >
            {exporting ? "Exporting…" : "Download PDF"}
          </button>
          <button onClick={onClose} aria-label="Close" style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.38)", cursor: "pointer", fontSize: 20, lineHeight: 1, padding: "2px 6px", display: "flex", alignItems: "center" }}>×</button>
        </div>
      </div>
      {exportErr && <div style={{ padding: "7px 16px", background: "rgba(239,68,68,0.1)", fontSize: 12, color: "#f87171", borderBottom: `1px solid ${BORDER}` }}>{exportErr}</div>}
      ...
      <div style={scrollCSS}>
        ...
        <PSection title="Recommendation">
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            ...
            <div style={{ background: ratingBg(rt.recommendation), border: `1px solid ${ratingColor(rt.recommendation)}44`, borderRadius: 8, padding: "10px 14px", textAlign: "center", flexShrink: 0 }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: ratingColor(rt.recommendation) }}>{rt.recommendation}</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", marginTop: 2 }}>{fmtP(rt.target_price)}</div>
              <div style={{ fontSize: 11, color: ratingColor(rt.recommendation), fontWeight: 600 }}>{fmtPct(rt.price_upside)}</div>
            </div>
          </div>
        </PSection>
        ...
        <PSection title="Fundamentals">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 6 }}>
            <MTile label="EPS"            value={fmtP(r.fundamental_snapshot.eps)} />
            <MTile label="Revenue Growth" value={fmtPct(r.fundamental_snapshot.revenue_growth)} />
            <MTile label="Profit Margin"  value={fmtPct(r.fundamental_snapshot.profit_margin)} />
            <MTile label="ROE"            value={fmtPct(r.fundamental_snapshot.roe)} />
            <MTile label="Debt/Equity"    value={r.fundamental_snapshot.debt_to_equity?.toFixed(2) ?? "N/A"} />
            <MTile label="Quality Score"  value={`${r.fundamental_snapshot.quality_score.toFixed(0)} / 100`} />
          </div>
          <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${BORDER_SUB}`, borderRadius: 6, padding: "8px 10px" }}>
        ...
        <PSection title="Technical Analysis">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
            ...
            <MTile label="Signal"       value={`${r.technical_view.signal_strength.toFixed(0)} / 100`} />
        ...
        <PSection title="Investment Cases">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div style={{ background: "rgba(34,197,94,0.05)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#4ade80", marginBottom: 5 }}>▲ Bull ({r.bull_case.probability.toFixed(0)}%)</div>
              <p style={{ fontSize: 11, color: "#94a3b8", lineHeight: 1.4, margin: "0 0 8px" }}>{r.bull_case.thesis}</p>
              <div style={{ height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
                <div style={{ height: 3, background: "#22c55e", borderRadius: 2, width: `${r.bull_case.probability}%` }} />
              </div>
            </div>
            <div style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#f87171", marginBottom: 5 }}>▼ Bear ({r.bear_case.probability.toFixed(0)}%)</div>
              <p style={{ fontSize: 11, color: "#94a3b8", lineHeight: 1.4, margin: "0 0 8px" }}>{r.bear_case.thesis}</p>
              <div style={{ height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
                <div style={{ height: 3, background: "#ef4444", borderRadius: 2, width: `${r.bear_case.probability}%` }} />
              </div>
            </div>
          </div>
        </PSection>
        ...
        <PSection title="Final Recommendation">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1, height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3 }}>
              <div style={{ height: 6, borderRadius: 3, background: "#22c55e", width: `${r.confidence_score}%`, transition: "width 0.6s ease" }} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#e2e8f0", flexShrink: 0 }}>{r.confidence_score.toFixed(0)}%</span>
          </div>
        </PSection>
    ...
    <div style={{ borderTop: `1px solid ${BORDER}`, background: BG, flexShrink: 0, padding: "12px 16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "8px 10px" }}>
        ...
        <button onClick={onSend} disabled={!canSend} aria-label="Send"
          style={{ background: canSend ? "#1d4ed8" : "rgba(255,255,255,0.07)", border: "none", borderRadius: 8, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: canSend ? "pointer" : "not-allowed", flexShrink: 0, transition: "background 0.15s" }}>
...
export default function AIAnalystPage() {
  ...
  const [rightPanelReport, setRightPanelReport] = useState<AnalystReport | null>(null);
  ...
  async function handleSend() {
    ...
    setMessages((prev) => [...prev, { id: userMsgId, role: "user", content: userText, attachments: sentAttach }]);

    // 2. Placeholder assistant message
    const asstId = genId();
    setMessages((prev) => [...prev, { id: asstId, role: "assistant", content: "", isTyping: true, reportLoading: false }]);

    // 3. Extract ticker
    const tickerMatch = userText.match(/\banalyze\s+([a-z]{1,5})\b/i);
    const ticker      = tickerMatch ? tickerMatch[1].toUpperCase() : null;

    // History = all messages before this turn
    const history = messages.map((m) => ({ role: m.role, content: m.content }));

    // 4. Mark report loading if ticker found
    if (ticker) {
      setMessages((prev) => prev.map((m) => m.id === asstId ? { ...m, reportLoading: true, reportTicker: ticker } : m));
    }
    ...
    try {
      ...
      setMessages((prev) => prev.map((m) => m.id === asstId ? { ...m, content: reply, isTyping: false } : m));
    } catch (err) {
      const msg = err instanceof Error ? err.message : "An error occurred.";
      setMessages((prev) => prev.map((m) => m.id === asstId ? { ...m, content: `⚠️ ${msg}`, isTyping: false, reportLoading: false } : m));
      setIsLoading(false);
      return;
    }
    ...
    if (reportFetch) {
      try {
        ...
        setMessages((prev) => prev.map((m) => m.id === asstId ? { ...m, reportLoading: false, reportData } : m));
        setRightPanelReport(reportData);
      } catch (_err) {
        setMessages((prev) => prev.map((m) => m.id === asstId ? { ...m, reportLoading: false } : m));
      }
    }
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes ai-slide-in { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        .ai-panel-slide { animation: ai-slide-in 0.2s ease-out; }
      ...
      <div style={{ display: "flex", height: "100vh", background: BG, color: "#e2e8f0", fontFamily: "Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          <div ref={threadRef} className="ai-thread" style={threadCSS}>
            {messages.length === 0
              ...
              : messages.map((msg) => <MessageBubble key={msg.id} msg={msg} onViewReport={setRightPanelReport} />)
            }
          </div>
          <InputBar
        ...
        {/* Right panel */}
        {rightPanelReport && (
          <RightPanel report={rightPanelReport} onClose={() => setRightPanelReport(null)} />
        )}
      </div>
    </>
  );
